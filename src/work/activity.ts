...
export function recentActivity(journal: AuditJournal, projectId: string, workspaceId?: string, afterSequence?: number) {
  ...
  const changes = journal.list({ projectId, ...(afterSequence === undefined ? {} : { afterSequence }), cursorRecovery: "recent", limit: 10, mutatingOnly: true });
  const warning = changes.cursor_recovery || changes.gap_detected ? {
    ...(changes.cursor_recovery ?? { reason: "journal_gap", mode: "recent_retained", oldest_safe_forward_cursor: null }),
    guidance: "Activity history is incomplete. This is a recent retained snapshot, not a complete replay since the checkpoint. Inspect current source, the saved handoff and durable operation/job receipts before continuing. Missing activity is not evidence that work did not happen."
  } : undefined;
  ...
  return { enabled: changes.enabled, last_recorded_project_change: changed ? { at: changed.finished_at, action_id: changed.action_id, workspace_id: changed.workspace_id, changed_paths: changed.changed_paths.slice(0, 10) } : null,
    ...
    latest_sequence: changes.latest_sequence, next_sequence: changes.next_sequence, earliest_sequence: changes.earliest_sequence, has_more: changes.has_more, gap_detected: changes.gap_detected || !!changes.cursor_recovery,
    ...(warning ? { warning } : {}),
