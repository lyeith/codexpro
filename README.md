...
```text
CODEXPRO_ASSET_DIR=.codexpro/assets
CODEXPRO_MAX_ASSET_BYTES=10000000
...
Signed asset URLs use `CODEXPRO_ASSET_SECRET` when set, otherwise `CODEXPRO_HTTP_TOKEN` when available, otherwise a per-process random secret. Restarting the server invalidates process-secret URLs, which is usually fine for short TTL asset links.
