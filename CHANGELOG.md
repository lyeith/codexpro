...
- Added an authenticated `/activity` operator page that groups recent metadata-only actions by configured project and pairs them with a bounded, safety-filtered working-tree diff against `HEAD`. Blocked paths and untracked file contents remain hidden, and the page never exposes raw shell command text.
