...
- Replaced the public exact-text `edit` contract with four-hex-tagged multi-hunk editing. `read` retains the exact full snapshot and displayed-line provenance per MCP session; `edit` accepts only that compact tag plus replace/delete/insert operations, validates collisions and stale content against the retained full bytes, freezes every hunk to the original line coordinates, and writes once after complete preflight. Line endings, BOMs, final-newline shape, inode metadata, and hard-link identity remain preserved.
