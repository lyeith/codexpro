...
- Added an authenticated `/activity` operator page that groups recent metadata-only actions by configured project and pairs them with a bounded, safety-filtered working-tree diff against `HEAD`. Actions now use expandable tool-specific cards: edits expose line deltas, replacements, changed paths, and file evidence; reads/searches expose ranges, sizes, and result counts; Bash exposes only an allowlisted command label plus execution metadata. Blocked paths, raw shell arguments, and untracked file contents remain hidden.
