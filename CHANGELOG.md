...
- Public activity records deliberately omit file bodies, prompts, search and shell command text, tokens, attachment bytes, stdout/stderr, raw identity values, absolute roots, and raw tool results. The private local journal now retains bounded exact Bash scripts solely for the authenticated activity dashboard, while list/get/export strip the complete private field. The journal and its internal files remain blocked from workspace tools and mode `0600` on POSIX systems.
...
- Fixed audit outcome classification so successful tool results that merely contain words such as `timeout` are not reported as timed out. Bash now emits an explicit bounded timeout flag, while generic timeout-text matching is restricted to actual tool errors. The stress fixture now opts into metadata auditing and handoff tools explicitly instead of relying on obsolete defaults.
- Added an authenticated `/activity` operator page that groups recent actions by configured project, renders bounded exact scripts for direct and executed batch Bash calls, and pairs them with a bounded, safety-filtered split working-tree diff against `HEAD`. Actions use expandable tool-specific cards; blocked paths and untracked file contents remain hidden, and all embedded text is HTML-escaped.
- Reduced default action-journal retention from 64 MiB/50,000 actions to 8 MiB and 200 actions per project (plus a separate unscoped bucket), and enforce the configured ceilings at HTTP startup as well as after appends. Busy projects can no longer evict concurrent projects; intentional cross-project sequence gaps are integrity-checked and expose an explicit safe cursor floor.
- Corrected the per-project retention cursor floor to the value immediately before the oldest complete contiguous retained suffix, instead of conservatively pinning it to the latest compacted action. Forward consumers now retain all usable post-gap history, and connectors transparently accept and normalize indexes written by the earlier conservative implementation.
- Replaced the public exact-text `edit` contract with four-hex-tagged multi-hunk editing. `read` retains the exact full snapshot and displayed-line provenance in a bounded process-wide cache isolated by connector principal, so HTTP transport/server rotation no longer invalidates an immediate read/edit sequence. `edit` validates collisions and stale content against the retained full bytes, freezes every hunk to the original line coordinates, and writes once after complete preflight. Line endings, BOMs, final-newline shape, inode metadata, and hard-link identity remain preserved.
- Added stable actionable failure metadata for tagged edits and patches: `error_code`, `retry_unchanged`, and a bounded `recovery` action. Patch handling now rejects harness wrapper syntax explicitly, distinguishes stale context from malformed raw Git diffs, and steers ordinary one-file changes back to tagged `edit` instead of encouraging unchanged retries.
- Added bounded `batch` operations for related workspace calls. Parallel execution is restricted to safe read tools; serial batches permit one `write`/`edit`/`apply_patch` followed by allowlisted test/typecheck/lint/build or Git-inspection Bash children, reads, and change review. Every child schema and batch policy is preflighted before execution; non-zero/timeout/signal Bash results fail the child; aggregate output is budgeted; and mutation evidence remains available to metadata-only audit.
- Inline batches containing actual Bash verification are normalized into ordinary editable JSON definitions under `.codexpro-batches/`; other inline batches remain one-shot unless `persist=true`. Tiny one- or two-operation non-verification batches return efficiency guidance instead of silently creating retained files. Stored definitions keep stable operation IDs, four-hex filenames, local Git exclusion, 0600 permissions, and retention of the 20 most recently created, amended, or run definitions per workspace. `batch(path=..., from=...)` and zero-based `from_index` resume without replaying the successful prefix.
- Added `open_workspace(project_ids=[...])` for resolving up to 12 named projects in one call. It deduplicates ids, preflights the entire request before opening anything, preserves input order, selects the first project as primary, skips multi-project trees by default, shares an explicit tree budget, and tells agents to reuse the returned workspace handles. Repeated singular opens are now compact by default while `include_tree=true` remains an explicit refresh.
- Documented the Oh My Pi research, MCP-specific design choices, and review-only follow-up candidates including structural search, read-only LSP, and preview-first structural rewrites in `docs/HASH_EDIT_AND_BATCH.md`.
