...
Commands: give bash a timeout_ms that covers the command (default 120 s) and read the result directly; do not set tiny timeouts to push work into the background. Use background=true only for genuinely long work you will keep working during. To collect a job, call jobs(job_id, wait_ms=120000) once and let it wait; never poll with short waits. Write files with write/edit, not with bash heredocs.
